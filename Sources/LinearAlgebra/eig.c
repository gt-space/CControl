...
#include "linearalgebra.h"
 ...
 /* Regular routines */
bool eig(const double A[], double dr[], double di[], double wr[], double wi[], size_t row);
...
bool eig(const double A[], double dr[], double di[], double wr[], double wi[], size_t row) {
	/* Create copy */
	size_t i = row * row * sizeof(double);
	double* A_copy = (double*)malloc(i);
	memcpy(A_copy, A, i);
	...
	if (symmetric) {
		...
		memset(wi, 0, row * row * sizeof(double));
		memset(di, 0, row * sizeof(double));
	}
	else {
		/* Settings */
		...
		/* Eigenvectors */
		doublereal* vl = (doublereal*)malloc(n * lda * sizeof(doublereal));
		doublereal* vr = (doublereal*)malloc(n * lda * sizeof(doublereal));

		/* Allocate memory */
		lwork = -1;
		dgeev_("V", "N", &n, A_copy, &lda, dr, di, vl, &ldvl, vr, &ldvr, &wkopt, &lwork, &info);
		lwork = (integer)wkopt;
		doublereal* work = (real*)malloc(lwork * sizeof(doublereal));

		/* Compute  */
		dgeev_("V", "N", &n, A_copy, &lda, dr, di, vl, &ldvl, vr, &ldvr, work, &lwork, &info);
