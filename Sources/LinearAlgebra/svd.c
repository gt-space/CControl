 ...
 * Return false = fail
 */
bool svd(const double A[], const size_t row, const size_t column, double U[], double S[], double V[]) {
	...
	if (symmetric) {
		...
		memcpy(a, A, lda * n * sizeof(doublereal));

		/* Allocate memory */
		lwork = -1;
		liwork = -1;
		dsyevd_("V", "U", &n, a, &lda, S, &wkopt, &lwork, &iwkopt, &liwork, &info);
		...
		/* Compute */
		dsyevd_("V", "U", &n, a, &lda, S, work, &lwork, iwork, &liwork, &info);
		...
		/* Move over to V */
		memcpy(V, U, row * row * sizeof(double));
